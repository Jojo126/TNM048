...
let zoom = d3v4.zoom()
	...
	.on("zoom", function () { gDraw.attr("transform", d3v4.event.transform); });
...
d3.json("data/data.json", function(error, graph) {
	...
	node.append("title").text(function(d) { return 'r/' + d.id; });
	...
	node.on("click", d => {
		...
		d.words.forEach(wordObj => {
			if (maxScore < wordObj.score) maxScore = wordObj.score;
			if (minScore > wordObj.score) minScore = wordObj.score;  
		});
...
function updateWordList() {
	...
	// Store all selected subreddits in an array
	...
	d3.json("data/data.json", function(error, dataSet) {
		...
		for(const subreddit of selectedSubreddits) {
			// Compare all node ids with the selected subreddits
			for (const node of dataSet.nodes) {
				// Subreddit found
				if (node.id == subreddit) {
					// Collect all word data
					for (const wordInfo of node.words) {
						// Check if the word is already in the word list
						let stored = wordList.get(wordInfo.word);

						// If word exists, add the word counts together
						if (stored) {
							wordList.set(wordInfo.word, {
							  score: wordInfo.score + stored.score, 
							  amount: wordInfo.amount + stored.amount 
							});
						} // If word don't exists, add values to the list
						else {
							wordList.set(wordInfo.word, {score: wordInfo.score, amount: wordInfo.amount});
						}
					}
				}
			}
		...
		let maxScore = 0,
			minScore = 999;
		for (const wordInfo of wordListArray) {
			if (maxScore < wordInfo[1].score) maxScore = wordInfo[1].score;
			if (minScore > wordInfo[1].score) minScore = wordInfo[1].score;
		}
		...
		for (const wordInfo of wordListArray) {
			(maxScore*maxScore > minScore*minScore) ? largestScore = maxScore : largestScore = minScore;
			let amountWidth = (100 / largestScore) * 100/2;
			
			let score = wordInfo[1].score;
			let amount = wordInfo[1].amount;
			let scoreWidth = score/100 * amountWidth;
			
			//Render list
			innerHTML += '<li><h3 class="word">' + wordInfo[0] + '</h3><div class="stapelCont"><h3 class="occurrences" title="occurrences: '+ amount +'">' + amount + '</h3><div class="stapel"><span class="background"></span>';
			// If positive score -> green span to left, otherwise red span to the right
			if (score >= 0) {
				innerHTML += '<span class="ups" title="score: ' + score + '" style="width: ' + scoreWidth + '%">' + score + '</span>';
			} else {
		...
		selectedSubreddits.forEach(title => {
...
function updateChart() {
	...
	let circle = d3.selectAll("circle").nodes().map(x => {
		...
		// Circle is green if in the selection, red otherwise (only for debugging purpose, see if correctly selected)
		if(isBrushed) x.style.opacity = 1;
		else x.style.opacity = 0.3;
	});
}
